...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	if function == "initCDR" { //create a new marble
		return t.initCDR(stub, args)
	...
	} else if function == "readCDR" { //read a marble
		return t.readCDR(stub, args)
...
// ============================================================
func (t *SimpleChaincode) initCDR(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	var err error
	...
	cdrAsBytes, err := stub.GetState(cdrID)
	if err != nil {
		return shim.Error("Failed to get cdr: " + err.Error())
	} else if cdrAsBytes != nil {
		fmt.Println("This cdr already exists: " + cdrID)
		return shim.Error("This cdr already exists: " + cdrID)
	}

	// ==== Create cdr object and marshal to JSON ====
	cdr := &cdr{callingNo, calledNo, duration, equipmentID, callResult, timestamp, cdrID}
	...
	err = stub.PutState(cdrID, cdrJSONasBytes)
	if err != nil {
		return shim.Error(err.Error())
	}
...
// ===============================================
func (t *SimpleChaincode) readCDR(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if err != nil {
		...
		jsonResp = "{\"Error\":\"CDR does not exist: " + name + "\"}"
		return shim.Error(jsonResp)
	}
...
func (t *SimpleChaincode) delete(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	valAsbytes, err := stub.GetState(cdrID) //get the marble from chaincode state
	if err != nil {
		jsonResp = "{\"Error\":\"Failed to get state for " + cdrID + "\"}"
		return shim.Error(jsonResp)
	} else if valAsbytes == nil {
		jsonResp = "{\"Error\":\"Marble does not exist: " + cdrID + "\"}"
		return shim.Error(jsonResp)
	}

	err = json.Unmarshal([]byte(valAsbytes), &cdrJSON)
	if err != nil {
		jsonResp = "{\"Error\":\"Failed to decode JSON of: " + cdrID + "\"}"
		return shim.Error(jsonResp)
	}

	err = stub.DelState(cdrID) //remove the marble from chaincode state
	if err != nil {
